...
*/
typedef struct{
    char* targets; // (target) Sequences (one after another)
	char* querys;
...
//--CPU functions---------------------------------------------------------------
inline void initializePenalties(Penalties* penalties, 
	int16_t match, int16_t mismatch, int16_t insertion, int16_t deletion)
{
	penalties->match = match;
	penalties->mismatch = mismatch;
	penalties->insertion = insertion;
	penalties->deletion = deletion;
}
...
inline void storeTarget(char* sequence, size_t size, DataStruct* data) {
	...
	size_t tsTotalSize = (data->targetRefs[data->nTargets])*sizeof(char);
	data->targets = (char*) realloc(data->targets, tsTotalSize);

	// Copy data to struct
	size_t tSize = size*sizeof(char);
	memcpy(&(data->targets[data->targetRefs[data->nTargets-1]]), sequence, tSize);
}

inline void storeQuery(char* sequence, size_t size, DataStruct* data) {
	// Update value (consider it for memory accesses)
	data->nQuerys++;
	...
	// Allocate memory for the new sequence
	size_t qsTotalSize = (data->queryRefs[data->nQuerys])*sizeof(char);
	data->querys = (char*) realloc(data->querys, qsTotalSize);

	// Copy data to the struct
	size_t qSize = size*sizeof(char);
...
inline void setMatrixesDataGPU(MatrixesData* matrixesStruct, DataStruct* data, uint32_t nAlignments) {
    ...
    */
	...
	size_t mTotalSize = totalRows*sizeof(int16_t*);
	matrixesAux = (int16_t**) malloc(mTotalSize);
	...
	for(uint32_t alIdx = 0; alIdx < nAlignments; alIdx++) {
		...
		size_t rSize = nCols*sizeof(int16_t);
...
inline void initializeResultsGPU(Results* resultStruct, uint32_t nAlignments) {
	size_t resSize = nAlignments*sizeof(int16_t);
...
//--GPU functions---------------------------------------------------------------
inline __device__ void storeResult(Results* resultStruct, uint32_t alIdx, int16_t result) {
...
inline void copySequencesToGPU(DataStruct* dataCPU, DataStruct* dataGPU) {
	...
	size_t tSize = dataCPU->targetRefs[nTargets]*sizeof(char);
	size_t qSize = dataCPU->queryRefs[nQuerys]*sizeof(char);
...
inline void copyResultsToCPU(Results* resultsCPU, Results* resultsGPU, uint32_t nAlignments){
	...
	resultsCPU->results = (int16_t*) malloc(resSize);
...
//--Kernel definitions----------------------------------------------------------
__global__ void nw_align_gpu(DataStruct sequences, Penalties penalties, MatrixesData matrixes, Results results);


//--Mem free functions----------------------------------------------------------
inline void freeCPU(DataStruct* data, Results* results) {
	// DataStruct free
	free(data->targets);
	free(data->querys);
...
inline void freeGPU(DataStruct* data, Results* results) {
	// DataStruct free
